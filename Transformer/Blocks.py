...
class EncodingBlock(torch.nn.Module):
    def __init__(self, emb_dim=512, attn_dim=64, n_head=8, dropout=0.0, feedforward_dim=1024, training=False):
        super().__init__()
        ...
        self.add_norm_1 = AddNorm(dropout=dropout, input_dim=emb_dim, training=training)
        self.ff = FeedForward(input_dim=emb_dim, hid_dim=feedforward_dim)
        self.add_norm_2 = AddNorm(dropout=dropout, input_dim=emb_dim, training=training)
    ...
    def forward(self, enc_input, mask=None):
        ...
        enc_output = self.add_norm_2(
            add_norm_output,
            F.relu(self.ff(add_norm_output))
        )
...
class DecodingBlock(torch.nn.Module):
    def __init__(self, emb_dim=512, attn_dim=64, n_head=8, dropout=0.0, feedforward_dim=1024, training=False):
        ...
        self.add_norm_2 = AddNorm(dropout=dropout, input_dim=emb_dim, training=training)
        self.ff = FeedForward(input_dim=emb_dim, hid_dim=feedforward_dim)
        self.add_norm_3 = AddNorm(dropout=dropout, input_dim=emb_dim, training=training)
